...
import { ChevronDown, Search, Lock, Globe, Loader2, X } from 'lucide-react'
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <div className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center gap-1">
          ...
          <button
            ...
            className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground"
            data-testid="repo-dropdown-toggle"
          >
            {isLoading ? (
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="max-h-60 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="py-6 text-center text-sm text-muted-foreground">
                {search ? 'No repositories found' : 'Type to search or enter URL'}
              </div>
            ) : (
              repos.map((repo) => (
                <button
          ...
          {value && !repos.some(r => r.cloneUrl === value) && (
            <div className="border-t p-2">
              <div className="text-xs text-muted-foreground px-2">
                Using custom URL: <span className="font-mono">{value}</span>
              </div>
            </div>
          )}
        </div>
